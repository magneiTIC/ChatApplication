...
module.exports = {
  async login(req, res) {
    try {
      ...
      console.log('passwordSalt',userRecord.passwordHash)
      if (password && userRecord.passwordSalt) {
        const isPasswordValid = await bcrypt.compare(password, userRecord.passwordSalt);
        console.log("isPassworddValid",  isPasswordValid );
