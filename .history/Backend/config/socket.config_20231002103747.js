...
module.exports = io => {
    io.on("connection", socket => {
        ...
        socket.on('chat-message',(messagData)=>{
            // Diffuser le message à tous les clients connectés
            //socket.broadcast.to(socket.id).emit('chat-message', message);
            console.log("Message : ", messagData.message, "de l'utilisateur avec id:", messagData.socketID)
            socket.broadcast.emit('chat-message', messageDa);
        });
